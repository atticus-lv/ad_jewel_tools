...
class ADJT_OT_SplitCurveAndFlowMesh(ADJT_OT_ModalTemplate):
    """For selected object which use active curve with muti splines to array and deform, copy it (retarget the modifier curve to the new split spline)
对于使用激活项曲线（具有多段样条）来进行阵列和变形的网格物体进行复制（并将修改器中曲线重定向到新的分裂样条）"""
